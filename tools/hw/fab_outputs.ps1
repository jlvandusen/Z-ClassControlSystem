...
# Produces hardware/fab/<yyyymmdd>/: gerbers.zip, drill files, pick-and-place CSV, BOM CSV, PDFs.
...
$pcb  = Join-Path $root 'hardware\kicad\mainboard.kicad_pcb'
$sch  = Join-Path $root 'hardware\kicad\mainboard.kicad_sch'
$out  = Join-Path $root ("hardware\fab\" + (Get-Date -Format 'yyyyMMdd'))
New-Item -ItemType Directory -Force $out | Out-Null
$g = Join-Path $out 'gerbers'; New-Item -ItemType Directory -Force $g | Out-Null

& $cli.FullName pcb export gerbers --output "$g\" --layers "F.Cu,B.Cu,F.Paste,B.Paste,F.SilkS,B.SilkS,F.Mask,B.Mask,Edge.Cuts" --subtract-soldermask --use-drill-file-origin $pcb
& $cli.FullName pcb export drill   --output "$g\" --format excellon --excellon-units mm --generate-map --map-format gerberx2 $pcb
...
& $cli.FullName sch export bom     --output (Join-Path $out 'bom_from_schematic.csv') --fields "Reference,Value,Footprint,${QUANTITY}" --group-by "Value,Footprint" $sch
