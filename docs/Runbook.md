...
- **The full sealed-ball service path (RC4.7).** Every drive console command runs over this bridge, so a closed ball never needs opening: `cfg autocenter`, the PID autotuner (`autotune drive|s2s` → `autotune apply` → `pid save`), and the six sign fixes (`pref rev*` / `inv*`, §8.6) all inject into the same parser the USB console uses. Caveat: **`cfg autocenter` drives the S2S to both stops for ~15–20 s** — it pumps the tunnel from inside that blocking sweep and streams a ~1 Hz "busy" heartbeat with the live pot, so you can see it working, then the compact `low/high/center` result lands as one line. The relay `autotune` is non-blocking (loop-serviced), so its progress and Ku/Tu results stream back normally. Pad must be connected (above).
